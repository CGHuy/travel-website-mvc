...
CREATE TRIGGER trg_users_code_ai
AFTER INSERT ON users
FOR EACH ROW
BEGIN
    UPDATE users
    SET user_code = CONCAT('USR', LPAD(NEW.id, 5, '0'))
    WHERE id = NEW.id;
END;
//

CREATE TRIGGER trg_tours_code_ai
AFTER INSERT ON tours
FOR EACH ROW
BEGIN
    UPDATE tours
    SET tour_code = CONCAT('TOUR', LPAD(NEW.id, 5, '0'))
    WHERE id = NEW.id;
END;
//

CREATE TRIGGER trg_departures_code_ai
AFTER INSERT ON tour_departures
FOR EACH ROW
BEGIN
    UPDATE tour_departures
    SET departure_code = CONCAT('DEP', LPAD(NEW.id, 5, '0'))
    WHERE id = NEW.id;
END;
//

CREATE TRIGGER trg_services_code_ai
AFTER INSERT ON services
FOR EACH ROW
BEGIN
    UPDATE services
    SET service_code = CONCAT('SRV', LPAD(NEW.id, 5, '0'))
    WHERE id = NEW.id;
END;
//

CREATE TRIGGER trg_bookings_code_ai
AFTER INSERT ON bookings
FOR EACH ROW
BEGIN
    UPDATE bookings
    SET booking_code = CONCAT('BOK', LPAD(NEW.id, 5, '0'))
    WHERE id = NEW.id;
END;
//

CREATE TRIGGER trg_reviews_code_ai
AFTER INSERT ON reviews
FOR EACH ROW
BEGIN
    UPDATE reviews
    SET review_code = CONCAT('RVS', LPAD(NEW.id, 5, '0'))
    WHERE id = NEW.id;
END;
//

CREATE TRIGGER trg_wishlist_code_ai
AFTER INSERT ON wishlist
FOR EACH ROW
BEGIN
    UPDATE wishlist
    SET wish_code = CONCAT('WISH', LPAD(NEW.id, 5, '0'))
    WHERE id = NEW.id;
END;
//
